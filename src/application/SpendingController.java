...
public class SpendingController implements Initializable {
	...
	@FXML private ChoiceBox <String> spendingChoiceBox;
	private String[] categoryOfSpending = 
		{"Food","Clothes","Drinks","DailySupplies","HairAndCosmetic","Gathering","Studying","Hobbies","Traffic","Health","Else"};
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		spendingChoiceBox.getItems().addAll(categoryOfSpending);
		spendingChoiceBox.setOnAction(this::getCategory);
	}
	
	public void getCategory(ActionEvent event) {
		categoryValue = (String) spendingChoiceBox.getValue();
	...
	public void pressToRecordTheData() {
		...
		cost = Integer.parseInt(moneyString);//把金額從String轉成int
	...
	public void judgeCategory() {
		if(categoryString.equals("Food")) 
			Food.addTotalOfMonth();
		else if(categoryString.equals("Clothes")) 
			Clothes.addTotalOfMonth();
		else if(categoryString.equals("Drinks")) 
			Drinks.addTotalOfMonth();
		else if(categoryString.equals("DailySupplies")) 
			DailySupplies.addTotalOfMonth();
		else if(categoryString.equals("HairAndCosmetic")) 
			HairAndCosmetic.addTotalOfMonth();
		else if(categoryString.equals("Gathering")) 
			Gathering.addTotalOfMonth();
		else if(categoryString.equals("Studying")) 
			Studying.addTotalOfMonth();
		else if(categoryString.equals("Hobbies")) 
			Hobbies.addTotalOfMonth();
		else if(categoryString.equals("Traffic")) 
			Traffic.addTotalOfMonth();
		else if(categoryString.equals("Health")) 
			Health.addTotalOfMonth();
		else if(categoryString.equals("Else")) 
			Else.addTotalOfMonth();
	}
